...
public class SparkEngineConf implements Serializable {
    ...
    private void writeObject(ObjectOutputStream out) throws IOException {
        ...
        if (!UDFContext.getUDFContext().isUDFConfEmpty()) {
            ...
            LOG.info("SparkEngineConf#SPARK_UDFCONTEXT_UDFCONFS");
            if(this.properties.getProperty(SPARK_UDFCONTEXT_UDFCONFS) == null ){
                LOG.info("null");
            }else{
                LOG.info("not null");
            }
            LOG.debug("SPARK_UDFCONTEXT_CLIENTSYSPROPS");
            if( this.properties.getProperty(SPARK_UDFCONTEXT_CLIENTSYSPROPS) == null ){
                LOG.info(" null");
            }else{
                LOG.info("not null");
            }
